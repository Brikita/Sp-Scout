...
type Props = {
  ...
  mode: "fixture" | "live"; onMode: (value: "fixture" | "live") => void; liveAvailable: boolean;
  ...
  return <form ref={formRef} onSubmit={(event) => { if (step < 2) { event.preventDefault(); next(); } else p.onSubmit(event); }}>
    ...
    <fieldset className="request-fields" disabled={p.locked || p.busy}>
      ...
      {step === 2 && <div className="wizard-fields">
        ...
        <fieldset className="mode-choice"><legend>How would you like to try Scout?</legend><label className={p.mode === "fixture" ? "selected" : ""}><input type="radio" name="execution-mode" aria-label="Safe fixture execution" checked={p.mode === "fixture"} onChange={() => p.onMode("fixture")} /><span><strong>Try the demo</strong><small>Three sample suppliers. No dialing.</small></span></label><label className={p.mode === "live" ? "selected" : ""}><input type="radio" name="execution-mode" aria-label="Live pilot execution" disabled={!p.liveAvailable} checked={p.mode === "live"} onChange={() => p.onMode("live")} /><span><strong>Live pilot</strong><small>{p.liveAvailable ? "Your approved supplier contacts." : "Available on the configured pilot server."}</small></span></label></fieldset>
